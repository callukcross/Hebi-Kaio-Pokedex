...
public sealed class DataManagementForm : Form
{
    ...
    public DataManagementForm(ProfileTransferService transfers, CustomContentService modules, Action profileChanged)
    {
        ...
        profileButtons.Controls.Add(Button("Copy Pokémon", CopyPokemon));
        profileButtons.Controls.Add(Button("Paste Pokémon", PastePokemon));
        profileButtons.Controls.Add(Button("Show QR", ShowQr));
        profileButtons.Controls.Add(Button("Read QR Image", ReadQr));
    ...
    private void CopyPokemon(object sender, EventArgs args)
    {
        var selected = PickPokemon("Copy"); if (selected is null) return;
        Run(() => Clipboard.SetText(_transfers.ExportPokemonText(selected.Id)), "Pokémon copied to the clipboard.");
    }

    private void PastePokemon(object sender, EventArgs args)
    {
        Run(() => { var pokemon = _transfers.ImportPokemonText(Clipboard.GetText()); MessageBox.Show(this, $"Imported {pokemon.Nickname ?? pokemon.SpeciesName} into storage.", "Import complete"); });
    }

    private void ShowQr(object sender, EventArgs args)
    {
        var selected = PickPokemon("Share"); if (selected is null) return; using var dialog = new PokemonQrForm(_transfers, selected); dialog.ShowDialog(this);
    }

    private void ReadQr(object sender, EventArgs args)
    {
        using var dialog = new OpenFileDialog { Filter = "QR image (*.png;*.jpg;*.jpeg;*.bmp)|*.png;*.jpg;*.jpeg;*.bmp" }; if (dialog.ShowDialog(this) != DialogResult.OK) return;
        Run(() => { var pokemon = _transfers.ImportPokemonQrPayload(PokemonQrForm.ReadPayload(dialog.FileName)); MessageBox.Show(this, $"Imported {pokemon.Nickname ?? pokemon.SpeciesName} into storage.", "QR import complete"); });
    }

    private OwnedPokemon PickPokemon(string action)
    {
        var pokemon = _transfers.GetExportablePokemon();
        if (pokemon.Count == 0) { MessageBox.Show(this, "The active profile has no Pokémon to share."); return null; }
        using var picker = new Form { Text = $"Choose Pokémon to {action}", ClientSize = new Size(420, 150), StartPosition = FormStartPosition.CenterParent, FormBorderStyle = FormBorderStyle.FixedDialog };
        var choice = new ComboBox { Left = 20, Top = 30, Width = 380, DropDownStyle = ComboBoxStyle.DropDownList };
        foreach (var item in pokemon) choice.Items.Add(new PokemonItem(item)); choice.SelectedIndex = 0;
        picker.Controls.AddRange([choice, new Button { Text = action, Left = 220, Top = 90, Width = 85, DialogResult = DialogResult.OK }, new Button { Text = "Cancel", Left = 315, Top = 90, Width = 85, DialogResult = DialogResult.Cancel }]);
        return picker.ShowDialog(this) == DialogResult.OK && choice.SelectedItem is PokemonItem selected ? selected.Pokemon : null;
    }
